...
- **CD Prisma behavior**: GitHub Actions deploy runs `prisma migrate deploy` on each enabled VPS instance (via `scripts/gha-deploy-instance-remote.sh`) after pulling the API image and before recreating services. The migrate step temporarily sets `DATABASE_URL=$DIRECT_URL` inside the container; **runtime API** still connects via PgBouncer using `DATABASE_URL` in `PrismaService`. Each instance `.env` must set `DIRECT_URL` when `DATABASE_URL` uses PgBouncer transaction pooler. Do not run `prisma migrate dev` in CD.
