...
- Trường nghiệp vụ chính:
  ...
  - **Expand (song song, payroll chưa đọc):** `allowance_per_block_per_student`, `max_allowance_per_block`, `student_tuition_per_block` — đơn giá mỗi block 30 phút, backfill `ROUND(giá_cũ / số_block_chuẩn)` từ lịch cố định; `null` khi không suy được số block. Dual-write khi ghi cột per-session. Xem ADR `docs/adr/2026-09-09-expand-block-pricing.md`.
...
- Các cột override học phí (nullable int):
  ...
  - `custom_tuition_per_block` (expand: học phí riêng mỗi block 30 phút; charge buổi vẫn đọc cột per-session; `null` khi không có override hoặc lớp không có số block chuẩn)
