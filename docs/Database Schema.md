...
- Trường nghiệp vụ chính:
  ...
  - **Expand (song song):** `allowance_per_block_per_student`, `max_allowance_per_block`, `student_tuition_per_block` — đơn giá mỗi block 30 phút, backfill `ROUND(giá_cũ / số_block_chuẩn)` từ lịch cố định; `null` khi không suy được số block. Dual-write khi ghi cột per-session. **Học phí học sinh không gói:** chỉ khi lớp `pricing_mode = per_block` thì charge buổi đọc `student_tuition_per_block × sessions.snapshot_block_count` (thiếu per-block hoặc số block thì fallback cột per-session). Lớp `per_session` luôn dùng chuỗi theo buổi. Trợ cấp gia sư / trần trợ cấp / gói học phí vẫn theo bước expand (#134); nhánh trợ cấp theo block do #135. Xem ADR `docs/adr/2026-09-09-expand-block-pricing.md`.
...
- `snapshot_block_count` (`INTEGER`, nullable): số block 30 phút của buổi tại thời điểm **tạo** (từ `start_time`/`end_time`, fallback số block chuẩn của lớp). Payroll lịch sử không được suy lại từ giờ buổi. Expand #134; công thức payroll vẫn dùng snapshot per-session.
