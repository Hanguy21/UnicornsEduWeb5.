...
- Trường nghiệp vụ chính:
  ...
  - **Expand (song song):** `allowance_per_block_per_student`, `max_allowance_per_block`, `student_tuition_per_block` — đơn giá mỗi block 30 phút, backfill `ROUND(giá_cũ / số_block_chuẩn)` từ lịch cố định; `null` khi không suy được số block. Dual-write khi ghi cột per-session. **Học phí học sinh không gói:** charge buổi đọc `student_tuition_per_block × sessions.snapshot_block_count` (thiếu per-block hoặc số block thì fallback cột per-session). Trợ cấp gia sư / trần trợ cấp / gói học phí vẫn theo bước expand (#134). Xem ADR `docs/adr/2026-09-09-expand-block-pricing.md`.
