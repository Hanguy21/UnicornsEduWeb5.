...
- `buttonClassName` và `menuClassName` của `UpgradedSelect` **đè lên** style mặc định qua `twMerge` (không thay thế nữa): truyền mỗi chiều rộng (`w-full sm:w-72`) hoặc `max-h-72` vẫn giữ nguyên viền/padding/nền/shadow chuẩn. Panel menu dùng `bg-bg-surface` đặc (không `/95` + `backdrop-blur`). Danh sách dài (khoá học, chương/chủ đề) nên bật `searchable` kèm `noResultsLabel`.
...
- Mutation trong phạm vi khoá/lớp invalidate key theo `courseId`/`classId` (`questionKeys.course`, `examLibraryKeys.course`, `courseKeys.chapters`, `courseKeys.difficultyLevelsPrefix`) qua helper `apps/web/lib/query-invalidation.ts` — không invalidate `*.all` khi đã biết phạm vi. Đề thi và chuyên đề luyện tập trên cây Nội dung là cùng hàng `Topic(kind = practice)` nhưng hai query key khác nhau: mutation đề thi **và** mutation topic trên `KnowledgeTreeCard` gọi `invalidateCoursePracticeTopicQueries(queryClient, courseId)` để invalidate cả `examLibraryKeys.course` lẫn `courseKeys.knowledgeTree`. Không dùng `staleTime: 0` để lách.
- Fetch `chapters` / `difficulty-levels` của khoá dùng hook chung `useCourseChapters` / `useCourseDifficultyLevels` (`apps/web/lib/hooks/`). Ô search server-side (BankPicker, `PracticeTopicQuestionsCard`, tab Đề thi workspace khoá) debounce ~300ms (`use-debounce`).
...
2. **Chế độ xác thực (khuyến nghị: OAuth client)** — mặc định khi **không** đặt `TAILSCALE_AUTH_MODE=authkey`:
   ...
   - **Màn “New credential” → Custom scopes (như UI Tailscale):** nút _Generate_ chỉ bật khi đã chọn ít nhất một scope. Với [`tailscale/github-action`](https://github.com/tailscale/github-action) runner cần credential **mint auth key** (scope kiểu `auth_keys` trong [tài liệu OAuth](https://tailscale.com/docs/features/oauth-clients)). Thực hành **least privilege:**
     - **Không** bật nhóm **General** (DNS, Policy File, Users, Services) — không cần cho bước deploy CI.
     - Mở **Keys** và bật quyền **Write** (hoặc đúng dòng mô tả _auth keys_ / tạo key) nếu UI hiển thị tách theo Read/Write.
...
2. **Mảng `ssh` (Tailscale SSH):** khi dùng `tailscale nc` tới cổng 22, policy **`ssh`** vẫn áp dụng. Với **`src` là tag** (ví dụ `tag:cicd`), Tailscale **không** cho dùng **`action: "check"`** — validator báo _"check action does not support tags in src"_. Dùng **`action: "accept"`** cho rule từ tag CI, ví dụ:
