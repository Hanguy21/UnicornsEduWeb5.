...
- Mutation trong phạm vi khoá/lớp invalidate key theo `courseId`/`classId` (`questionKeys.course`, `examLibraryKeys.course`, `courseKeys.chapters`, `courseKeys.topicsPrefix`, `courseKeys.difficultyLevelsPrefix`) qua helper `apps/web/lib/query-invalidation.ts` — không invalidate `*.all` khi đã biết phạm vi. Chuyên đề luyện tập cấp khoá vẫn có thể đọc qua `examLibraryKeys` (lần giao lớp) và `courseKeys.topics`; mutation gọi `invalidateCoursePracticeTopicQueries(queryClient, courseId)`. Không dùng `staleTime: 0` để lách.
