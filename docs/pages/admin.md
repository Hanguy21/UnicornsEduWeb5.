...
- **Calendar tuần:** `/admin/calendar` dùng `Class.schedule` làm source of truth và chỉ hiển thị lịch học của **tuần hiện tại** theo layout week-view kiểu Google Calendar, cố định từ **CN -> T7**; FE giữ filter lớp dưới dạng **combobox tìm theo tên** và thêm filter single-select theo **gia sư chịu trách nhiệm** của từng slot lịch, không dùng pagination hay chọn khoảng ngày thủ công. Event card và popup không còn hiển thị gia sư của từng buổi; nếu lớp đã sync recurring event lên Google Calendar thì popup đọc trực tiếp `meetLink` từ schedule occurrence để mở link lớp và có thêm icon copy nhỏ để sao chép nhanh link Meet. Google Calendar chỉ áp dụng cho recurring schedule của lớp, không sync riêng các `session` được tạo/sửa/xóa từ màn lớp hoặc staff. Trên mobile, week-view giữ đủ 7 ngày bằng vùng cuộn ngang có chủ đích, filter xếp dọc mobile-first, và popup sự kiện chuyển sang kiểu bottom-sheet cuộn nội bộ để không tràn viewport.
...
- **Staff endpoints & frontend data fetching:**
  ...
  - `GET /staff/:id` trả thêm `classAllowance` (tổng hợp theo `class_id` + `teacher_payment_status`) để FE render cột `Tổng trước thuế / Chưa nhận trước thuế / Đã nhận trước thuế` theo lớp phụ trách.
...
- FE `/admin/staff/:id` đã bỏ block **Trước khấu trừ** khỏi card **Thống kê thu nhập** để giữ card gọn theo hướng net-first.
- FE `/admin/staff/:id` thêm card riêng **Thống kê thuế theo role** (đặt ngay dưới card **Công việc khác**), hiển thị mức thuế hiện hành theo từng role của nhân sự ở tháng đang xem, ưu tiên staff override rồi fallback role default từ `GET /deduction-settings/tax`; card dùng một nút **Chỉnh sửa** ở góc phải, bật chế độ bulk edit để sửa nhiều role cùng lúc ngay tại cột `Thuế hiện hành`, và nếu role chưa có override thì thao tác lưu sẽ tạo override mới cho staff.
  ...
  - FE `/admin/staff/:id` card **Lớp phụ trách** hiển thị `Tổng trước thuế / Chưa nhận trước thuế / Đã nhận trước thuế`; với gia sư đây là số sau khấu trừ vận hành nhưng trước khấu trừ thuế.
