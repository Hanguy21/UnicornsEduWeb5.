...
- **Staff endpoints & frontend data fetching:**
  ...
  - `PATCH /staff` (update staff) hỗ trợ cập nhật thêm `status` (`active|inactive`) và `customer_care_managed_by_staff_id` (UUID trợ lí quản lí, nullable) từ form “Chỉnh sửa thông tin nhân sự”. Backend validate staff được trỏ phải có role `assistant`.
