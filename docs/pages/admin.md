# Admin – `/admin/dashboard`, `/admin/notification`, `/admin/deductions`
...
- **Dashboard:** route canonical là `/admin/dashboard`; `/admin` là alias render cùng nội dung để tránh lỗi dev runtime khi đo performance trên redirect page. Dashboard đồng bộ layout tối giản: thanh lọc thời gian + nút xuất, cụm KPI card ngang, bảng **Báo cáo tài chính** (card trắng, tiêu đề trái, bảng 3 cột **Danh mục / Giá trị / Ghi chú** với header chữ nhỏ xám xanh, dòng mỏng; chín dòng: Tổng nạp, Học phí đã học, Nợ học phí chưa dạy, Chưa thu, Chờ thanh toán trợ cấp, Chi phí nhân sự, Chi phí khác, Lợi nhuận, Tổng nhận), section **Cảnh báo & hành động** dạng card group, và section **Chế độ xem nhanh theo phân hệ** (tab Tài chính/Vận hành/Học viên + chọn năm). Dữ liệu lấy thật từ backend qua `GET /dashboard`; frontend dùng TanStack Query (`apps/web/lib/apis/dashboard.api.ts`) và chỉ render dữ liệu aggregate authoritative từ BE. Mọi giá trị tiền trong bảng tài chính hiển thị dạng link (màu xanh) để mở popup chi tiết theo đúng từng dòng. Từ breakpoint mobile trở xuống, khối **Báo cáo tài chính** đổi sang danh sách card 1 cột để tránh scroll ngang; từ `md` trở lên giữ desktop table 3 cột.
  - Dòng **Học phí đã học** hiển thị số lũy kế của toàn bộ attendance `present` và `excused` trên hệ thống; phần ghi chú nêu thêm số học phí của tháng đang xem để không mất ngữ cảnh bộ lọc thời gian.
...
- **Reusable components (session history):** Dùng chung `SessionHistoryTable` + `SessionHistoryTableSkeleton` cho `/admin/classes/:id` và `/admin/staffs/:id`; cột thực thể được điều khiển bởi `entityMode` (`teacher` | `class` | `none`).
...
- **Dashboard aggregate endpoint (admin-only):**
  ...
  - `summary.totalLearnedTuition` lấy từ tổng `attendance.tuition_fee` của toàn bộ attendance `status IN (present, excused)` trên hệ thống; FE dùng field này cho dòng **Học phí đã học**.
  - `summary.monthlyRevenue` và line chart doanh thu lấy từ tổng `attendance.tuition_fee` của attendance `status IN (present, excused)`, gom theo `sessions.date`.
...
- **Users/Student/Staff endpoints (dùng qua FE api hooks ở `apps/web/lib/apis/user.api.ts`, `apps/web/lib/apis/staff.api.ts` và `apps/web/lib/apis/student.api.ts`):**
  ...
  - Khi `roleType=student`, popup hiển thị đầy đủ nhóm thông tin học sinh (cơ bản + phụ huynh + học tập) và danh sách lớp để gán ngay lúc tạo; class picker tìm lớp trực tiếp qua `GET /class` theo từ khóa nhập, giữ lại các lớp đã chọn trong UI và không còn hidden filter chỉ lấy lớp `running`. FE gọi `POST /users/student` qua TanStack Query mutation và hiển thị kết quả bằng Sonner toast.
...
- **Staff endpoints & frontend data fetching:**
  ...
  - `GET /staff/:id/income-summary?month=<01-12>&year=<YYYY>&days=<number>` trả summary thu nhập authoritative từ BE cho card tổng tháng/tổng năm theo hướng **net-first**:
    - `monthlyIncomeTotals` và `sessionMonthlyTotals` là số **thực nhận sau khấu trừ**.
    - thuế được tính trên **tổng thu nhập của từng nguồn trong kỳ** theo từng snapshot rate bucket; riêng nguồn **Giáo viên** tính thuế trên phần **sau khấu trừ vận hành**, không tính trực tiếp trên gross; **bonus không chịu thuế**.
    ...
    - `Tổng năm` tiếp tục dùng `yearIncomeTotal` (net), còn `yearGrossIncomeTotal` là tổng trước khấu trừ.
    - với gia sư, `classMonthlySummaries`, `recent unpaid`, `Ghi cọc` và `GET /staff/:id.classAllowance` dùng số **sau khấu trừ vận hành, trước thuế**; thuế chỉ hiển thị ở các summary tháng/năm.
    - khấu trừ thuế **không áp dụng cho bonus**; bonus vẫn giữ nguyên giá trị gốc trong tổng hợp backend.
  - `GET /deduction-settings/tax?asOfDate=<YYYY-MM-DD>&roleType=<staff-role>&staffId=<uuid>`
    - trả cấu hình khấu trừ thuế đang hiệu lực + lịch sử cho role defaults và staff overrides; deductions center dùng phần role defaults, còn staff detail dùng cùng endpoint để hiển thị và chỉnh override theo staff. Route này chỉ mở cho `admin`, `assistant`, `accountant`.
  - `POST /deduction-settings/tax/role-defaults` (append row mới theo `roleType`, `ratePercent`, `effectiveFrom`; chỉ mở cho `admin`, `assistant`, `accountant`)
  - `PATCH /deduction-settings/tax/role-defaults/:id` (chỉnh trực tiếp `ratePercent`, `effectiveFrom` của role default hiện có; không đổi `roleType`; chỉ mở cho `admin`, `assistant`, `accountant`)
  - `POST /deduction-settings/tax/staff-overrides` (tạo override theo staff từ card thuế ở `/admin/staffs/:id`; chỉ mở cho `admin`, `assistant`, `accountant`)
  - `PATCH /deduction-settings/tax/staff-overrides/:id` (chỉnh trực tiếp `ratePercent`, `effectiveFrom` của staff override hiện có từ card thuế ở `/admin/staffs/:id`; không đổi `staffId` và `roleType`; chỉ mở cho `admin`, `assistant`, `accountant`)
  - `POST /deduction-settings/tax/staff-overrides/bulk-upsert` (upsert nhiều override theo role trong một request; FE bulk edit trên staff detail dùng endpoint này để tránh N request; chỉ mở cho `admin`, `assistant`, `accountant`)
  - Session allowance trong `income-summary` được tính ở DB layer theo công thức `min(max_allowance_per_session, (allowance_amount * attended_students + scale_amount) * coefficient)`, trong đó `attended_students` đếm cả attendance `present` và `excused`.
  - `POST /staff` dùng để tạo hồ sơ nhân sự từ `user_id`; payload bắt buộc thêm `cccd_number` (12 chữ số, unique), optional `cccd_issued_date`, `cccd_issued_place`. Với luồng thêm gia sư từ admin page, FE gửi `roles=["teacher"]`.
  ...
  - `PATCH /staff` (update staff) hỗ trợ cập nhật thêm `status` (`active|inactive`), `cccd_number`, `cccd_issued_date`, `cccd_issued_place` và `customer_care_managed_by_staff_id` (UUID trợ lí quản lí, nullable) từ form “Chỉnh sửa thông tin nhân sự”. Backend validate staff được trỏ phải có role `assistant` và trả lỗi thân thiện khi `cccd_number` bị trùng.
  - `GET /staff/:id` trả thêm `customerCareManagedByStaffId` và `customerCareManagedBy: { id, fullName }` khi nhân sự có trợ lí quản lí.
  - `GET /staff/:id/payment-preview?month=<01-12>&year=<YYYY>` trả payload popup **Thanh toán** cho tháng đang xem, group theo `role -> source -> items`; mỗi item có `grossAmount`, `operatingAmount`, `taxRatePercent`, `taxAmount`, `netAmount`, và response có thêm `taxAsOfDate`. Thuế ở popup luôn được tính lại theo **mức hiện hành của chính staff đang thanh toán** cho từng role; riêng **Giáo viên** dùng base thuế = `gross - operating`. Bonus luôn nằm ở section riêng `Thưởng`, còn session `deposit` bị loại khỏi preview.
  ...
  - `PATCH /staff/:id/payment-status/pay-all` nhận body `{ month, year }`, tự tính lại toàn bộ khoản pending của tháng trong backend rồi chuyển sang `paid` trong một transaction duy nhất. Trước khi đổi trạng thái, backend refresh snapshot thuế theo **mức hiện hành của staff cho đúng role tương ứng**; riêng **Giáo viên** khi preview/pay-all và summary sẽ tính thuế trên phần sau vận hành. Endpoint này cập nhật đồng thời `session.teacher_payment_status`, `session.teacher_tax_deduction_rate_percent`, `bonus.status`, `attendance.customer_care_payment_status`, `attendance.customer_care_tax_deduction_rate_percent`, `attendance.assistant_payment_status`, `attendance.assistant_tax_deduction_rate_percent`, `lesson_outputs.payment_status`, `lesson_outputs.tax_deduction_rate_percent`, `extra_allowances.status`, `extra_allowances.tax_deduction_rate_percent`.
...
- FE `/admin/staff/:id` và `/admin/classes/:id`: bulk action bar (thanh hành động chọn nhiều) giờ **chỉ hiện khi đã tick chọn ít nhất 1 buổi**; checkbox tick chuyển trạng thái được tối giản theo style minimal để đồng bộ UI/UX toàn hệ thống.
  ...
  - FE `/admin/staff/:id` đổi cụm số liệu thu nhập từ bảng ngang sang **card grid** (Thực nhận tháng, Chưa nhận, Đã nhận, Tổng năm, Ghi cọc) để đọc nhanh hơn ở cả mobile/desktop.
- FE `/admin/staffs/:id` (re-export tại `/staff/staffs/:id`): trong **Thống kê thu nhập**, khối **Trước khấu trừ** (tổng tháng/chưa nhận/đã nhận trước khấu trừ, khấu trừ thuế, khấu trừ vận hành khi có) chỉ hiển thị khi người xem là **admin** hoặc **kế toán**; dữ liệu lấy từ cùng `GET /staff/:id/income-summary` (`monthlyGrossTotals`, `monthlyTaxTotals`, …) như trang `/staff/profile`.
- FE `/admin/staff/:id` thêm card riêng **Thống kê thuế theo role** (đặt ngay dưới card **Công việc khác**), hiển thị mức thuế hiện hành theo từng role của nhân sự ở tháng đang xem, ưu tiên staff override rồi fallback role default từ `GET /deduction-settings/tax`; card dùng một nút **Chỉnh sửa** ở góc phải, bật chế độ bulk edit để sửa nhiều role cùng lúc ngay tại cột `Thuế hiện hành`, và nếu role chưa có override thì thao tác lưu sẽ tạo override mới cho staff.
  - FE `/admin/staff/:id` thêm nút **Thanh toán** ở góc trên bên phải header. Popup thanh toán lấy dữ liệu từ `GET /staff/:id/payment-preview?month=&year=` theo tháng đang xem, group khoản pending theo từng role và source, đồng thời tách **Thưởng** thành section riêng thay vì ép map vào role.
  ...
  - Popup **Thanh toán** hiển thị chi tiết từng khoản theo cột động: phần **Giáo viên** giữ đủ **Trước thuế / Vận hành / Thuế / Sau thuế**; các role khác ẩn cột **Vận hành**; riêng section **Thưởng** chỉ còn **Trước thuế / Sau thuế**. Các session trạng thái `deposit` không được đưa vào popup này.
  - Khi xác nhận, FE gọi `PATCH /staff/:id/payment-status/pay-all` với body `{ month, year }`; backend tự tính lại tập pending trong transaction, refresh snapshot thuế hiện hành theo role cho các khoản được liệt kê, rồi chuyển tất cả sang trạng thái `paid`. FE không gửi danh sách id từ client và sẽ invalidate lại `staff detail`, `income-summary`, `sessions`, `bonus list`, `lesson output stats` sau khi thành công.
  ...
  - FE `/admin/staff/:id` card **Lớp phụ trách** hiển thị cột `Tổng / Chưa nhận / Đã nhận`; với gia sư đây là số sau khấu trừ vận hành nhưng trước khấu trừ thuế.
  - FE `/admin/staff/:id` bảng **Công việc khác** lấy số tổng hợp trực tiếp từ `GET /staff/:id/income-summary`; CSKH được cộng từ attendance commission theo `customer_care_payment_status`, giáo án được cộng từ `lesson_outputs.cost` + `lesson_outputs.payment_status`, assistant/communication/technical/accountant được cộng từ `extra_allowances.amount` theo `roleType + status`; ngoài ra role `assistant` còn được cộng thêm 3% học phí đã học (attendance `present` hoặc `excused`) từ CSKH mà trợ lí quản lí, aggregate qua `assistant_manager_staff_id` + `assistant_payment_status` trên bảng attendance. Riêng role `communication` và `technical` đều là extra-allowance-backed tax-only role; bonus có `workType = Truyền thông` vẫn nằm riêng ở card **Thưởng**. Nếu request `income-summary` lỗi, section này hiển thị inline error thay vì empty state. Khi role là CSKH (`customer_care`), bấm vào dòng (desktop) hoặc thẻ (mobile) sẽ chuyển sang `/admin/customer_care_detail/:id` (cùng staff id); khi role là `lesson_plan` hoặc `lesson_plan_head`, sẽ chuyển sang `/admin/lesson_plan_detail/:id`; khi role là `assistant`, route đích là `/admin/assistant_detail?staffId=:id`; khi role là `accountant`, route đích là `/admin/accountant_detail?staffId=:id`; khi role là `communication`, route đích là `/admin/communication_detail?staffId=:id`; khi role là `technical`, route đích là `/admin/technical_detail?staffId=:id`.
- **Customer-care detail (FE `/admin/customer_care_detail/[staffId]`):**
  ...
  - Khi mở rộng một học sinh ở tab **Hoa Hồng**, FE hiển thị từng buổi học theo đúng **một dòng / một hàng** dạng ledger ở desktop, còn mobile/tablet dùng card tách trường; cả hai layout đều có **badge trạng thái thanh toán CSKH** lấy trực tiếp từ `customerCarePaymentStatus` (`pending`/`paid`), kèm lớp, học phí, hệ số CSKH và số tiền hoa hồng của buổi đó.
  ...
  - FE `/admin/staffs/:id` không còn tổng hợp tài chính nặng ở client. Các card `Thống kê thu nhập`, `Lớp phụ trách`, `Thưởng`, `Công việc khác` lấy trực tiếp từ `GET /staff/:id/income-summary`; FE chỉ render summary và invalidate query này sau khi sửa session/bonus.
...
- **Class list (FE `/admin/classes`):** Hiển thị các cột chính: Tên lớp, Loại lớp, Sĩ số; dấu chấm trạng thái ở đầu mỗi dòng (running = warning, ended = muted). Dùng TanStack Query gọi `GET /class` qua `apps/web/lib/apis/class.api.ts`; thanh tìm kiếm chỉ search toàn bộ lớp theo tên và không còn filter loại lớp trên UI. Click dòng → `/admin/classes/:id`. Nút **Thêm lớp học** mở popup **Thêm lớp** (UI tối giản: khối gia sư, học sinh, học phí với **Tổng gói** + **Số buổi** và một dòng mức/buổi khi nhập hợp lệ, lịch học); submit `POST /class` gửi `student_tuition_per_session` làm tròn từ gói; success toast + đóng popup + invalidate `['class','list']`. Danh sách hỗ trợ phân trang theo `page` (URL query), điều hướng Trước/Sau, đồng bộ `page` theo `meta.page` từ backend và hiển thị phạm vi bản ghi hiện tại. Hành động xóa lớp dùng popup confirm nội bộ của app thay vì browser confirm/window popup.
- **Class endpoints (CRUD + pagination):**
  ...
  - `PATCH /class/:id/teachers` — thay thế danh sách gia sư (body: `{ teachers: [{ teacher_id, custom_allowance?, operating_deduction_rate_percent? }] }`).
    ...
    - FE hiện gửi kèm key tương thích `tax_rate_percent` trong giai đoạn chuyển đổi contract backend.
    - tỷ lệ khấu trừ vận hành được cấu hình theo từng cặp `teacher-class` và được snapshot xuống session qua `sessions.teacher_tax_rate_percent` khi tạo/cập nhật buổi học.
  - `PATCH /class/:id/schedule` — thay thế khung giờ học (body: `{ schedule: [{ dayOfWeek, from, to, teacherId, id? }] }`, HH:mm:ss). Mỗi slot phải có đúng 1 `teacherId` thuộc danh sách gia sư hiện có của lớp; backend tự preserve `googleCalendarEventId`/`meetLink` cũ theo `id` rồi sync lại recurring event nếu slot thay đổi.
...
- FE `/admin/classes/:id` hỗ trợ điều hướng nhanh: bấm vào **dòng gia sư phụ trách** → `/admin/staffs/:id`, bấm vào **dòng học sinh** → `/admin/students/:id`.
  ...
  - Card **Lịch dạy bù** hiển thị danh sách buổi bù **gần nhất trước**, có phân trang ngay trong card; admin/assistant có thể tạo, sửa, xoá buổi bù ngay tại đây. Dropdown `gia sư phụ trách` chỉ lấy từ `teachers` hiện có của lớp; nếu lớp chưa có gia sư thì nút thêm bị khoá.
  - Card **Khung giờ học** hiển thị luôn `gia sư chịu trách nhiệm` của từng slot. Popup chỉnh sửa khung giờ thêm dropdown chọn gia sư cho từng dòng; lựa chọn bị khóa trong phạm vi `teachers` của lớp, còn backend tự giữ `googleCalendarEventId`/`meetLink` cũ theo `id` để không mất liên kết recurring meeting đã sync.
  ...
  - Trong tab lịch sử buổi học của `/admin/classes/:id`, FE không còn hiển thị tên gia sư ở từng session row/card; thông tin buổi tập trung vào ngày, giờ, ghi chú, trạng thái và các chỉ số liên quan.
- FE `/admin/classes/:id` khu vực **Lịch sử & Khảo sát** được đồng bộ lại UI/UX theo backup: tab header dạng underline, thanh điều khiển trên cùng gồm **tổng buổi/tổng khảo sát** + cụm chọn tháng ở giữa + nút `+` bên phải, và tab Lịch sử bật thao tác chọn nhiều buổi để đổi nhanh trạng thái thanh toán ngay trong cùng khu vực.
  ...
  - Tab Lịch sử: nút "Thêm buổi học" mở popup form tối giản (`max-w-3xl`): header hiển thị đồng thời 2 thông tin: **Học phí** (màu xanh, chỉ hiện cho `admin`/`accountant`) và **Trợ cấp gia sư** (màu text chính, hiện cho mọi role); trường **Ngày học**, **Thời gian** (bắt đầu → kết thúc + dòng thời lượng), **Gia sư dạy**, **Hệ số**, **Trợ cấp** (admin để trống sẽ auto-fill theo công thức trợ cấp và hiển thị ghi chú công thức ngay dưới ô nhập), **Trạng thái thanh toán** (read-only mặc định chưa thanh toán khi tạo, nếu có quyền tài chính), **Nhận xét** (rich text), **Điểm danh** (bảng: trạng thái nhanh bằng icon, tên, ghi chú; mặc định khởi tạo là `vắng`; dòng tổng `Học / Phép / Vắng` bên dưới). Nếu lớp có đúng `1` gia sư thì khóa gia sư; nhiều gia sư thì chọn được. Preview học phí (khi bật) cộng `present` + `excused`. Submit `POST /sessions`.
  ...
  - **Chỉnh sửa buổi học:** Khi truyền `onSessionUpdated`, bảng lịch sử buổi học cho phép bấm **toàn bộ dòng/card session** để mở dialog chỉnh sửa (vẫn giữ icon thao tác ở cột cuối). Header dialog cũng hiển thị 2 thông tin tương tự form thêm: **Học phí** (xanh, chỉ `admin`/`accountant`) và **Trợ cấp gia sư** (màu text chính, mọi role). Form chỉnh sửa gồm ngày học, gia sư phụ trách, giờ bắt đầu/kết thúc, ghi chú (rich text), trạng thái thanh toán, **điểm danh học sinh** (khi có `getClassStudents`). Danh sách gia sư: từ trang lớp truyền `teachers`; từ trang gia sư truyền `getTeachersForClass(classId)`. Danh sách học sinh để điểm danh: truyền `getClassStudents(classId)` (trang lớp trả từ `classDetail.students`, trang gia sư gọi `GET /class/:id` lấy students). API list session (`GET /sessions/class/:id`, `GET /sessions/staff/:id`) trả thêm `attendance` cho mỗi session. Các card preview tài chính trong popup chỉnh sửa cộng attendance `present` và `excused`; chỉ `absent` không góp vào tổng hiển thị. Gọi `PUT /sessions/:id` với `date`, `teacherId`, `startTime`, `endTime`, `notes`, `teacherPaymentStatus`, `attendance`; backend tính học phí cho `attendance.status IN (present, excused)`. Khi buổi đã thanh toán (`paid`) hoặc đã cọc (`deposit`), danh sách điểm danh hiển thị đúng theo bản ghi attendance đã lưu của buổi (kèm tên học sinh từ API), không merge roster hiện tại của lớp. Buổi chưa thanh toán (`unpaid`) vẫn merge danh sách học sinh lớp hiện tại. Sau khi thành công invalidate query sessions. **Xóa buổi học:** cùng cột Thao tác có nút xóa (icon thùng rác), bấm vào mở popup xác nhận nội bộ; xác nhận thì gọi `DELETE /sessions/:id`, thành công thì toast và invalidate query sessions.
...
- **Giáo án admin (FE `/admin/lesson-plans`, alias `/admin/lessons`):**
  ...
  - Trong staff shell, cùng route `/staff/lesson-plans` sẽ tự đổi policy theo role: `lesson_plan_head` thấy 3 tab như manager, `lesson_plan` thấy `Tổng quan + Công việc` ở participant scope, còn `accountant` chỉ thấy tab `Công việc`; các route legacy `/staff/lesson-plan-tasks*` chỉ còn redirect về workspace mới.
  ...
  - Bảng `Resources` trong tab `Tổng quan`: với role quản lí (`admin`, `lesson_plan_head`, `assistant`), bấm trực tiếp vào card/dòng tài nguyên sẽ mở popup chỉnh sửa ngay tại chỗ; link ngoài và nút xóa vẫn là action độc lập, không bị đổi thành mở popup.
