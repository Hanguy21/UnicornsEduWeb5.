...
- **Calendar tuần:** `/admin/calendar` dùng `Class.schedule` làm source of truth và chỉ hiển thị lịch học của **tuần hiện tại** theo layout week-view kiểu Google Calendar, cố định từ **CN -> T7**; FE giữ filter lớp dưới dạng **combobox tìm theo tên** và thêm filter single-select theo **gia sư chịu trách nhiệm** của từng slot lịch, không dùng pagination hay chọn khoảng ngày thủ công. Event card và popup không còn hiển thị gia sư của từng buổi; nếu lớp đã sync recurring event lên Google Calendar thì popup đọc trực tiếp `meetLink` từ schedule occurrence để mở link lớp. Google Calendar chỉ áp dụng cho recurring schedule của lớp, không sync riêng các `session` được tạo/sửa/xóa từ màn lớp hoặc staff. Trên mobile, week-view giữ đủ 7 ngày bằng vùng cuộn ngang có chủ đích, filter xếp dọc mobile-first, và popup sự kiện chuyển sang kiểu bottom-sheet cuộn nội bộ để không tràn viewport.
...
- **Staff endpoints & frontend data fetching:**
  ...
  - `GET /deduction-settings/tax?asOfDate=<YYYY-MM-DD>&roleType=<staff-role>&staffId=<uuid>`
    - trả cấu hình khấu trừ thuế đang hiệu lực + lịch sử cho role defaults và staff overrides; deductions center dùng phần role defaults, còn staff detail dùng cùng endpoint để hiển thị và chỉnh override theo staff.
  - `POST /deduction-settings/tax/role-defaults` (append row mới theo `roleType`, `ratePercent`, `effectiveFrom`)
  - `PATCH /deduction-settings/tax/role-defaults/:id` (chỉnh trực tiếp `ratePercent`, `effectiveFrom` của role default hiện có; không đổi `roleType`)
  - `POST /deduction-settings/tax/staff-overrides` (tạo override theo staff từ card thuế ở `/admin/staffs/:id`)
  - `PATCH /deduction-settings/tax/staff-overrides/:id` (chỉnh trực tiếp `ratePercent`, `effectiveFrom` của staff override hiện có từ card thuế ở `/admin/staffs/:id`; không đổi `staffId` và `roleType`)
  - `POST /deduction-settings/tax/staff-overrides/bulk-upsert` (upsert nhiều override theo role trong một request; FE bulk edit trên staff detail dùng endpoint này để tránh N request)
...
- **Class endpoints (CRUD + pagination):**
  ...
  - `PATCH /class/:id/schedule` — thay thế khung giờ học (body: `{ schedule: [{ dayOfWeek, from, to, teacherId, id?, calendarEventId?, meetLink? }] }`, HH:mm:ss). Mỗi slot phải có đúng 1 `teacherId` thuộc danh sách gia sư hiện có của lớp.
...
- FE `/admin/classes/:id` hỗ trợ điều hướng nhanh: bấm vào **dòng gia sư phụ trách** → `/admin/staffs/:id`, bấm vào **dòng học sinh** → `/admin/students/:id`.
  ...
  - Card **Khung giờ học** hiển thị luôn `gia sư chịu trách nhiệm` của từng slot. Popup chỉnh sửa khung giờ thêm dropdown chọn gia sư cho từng dòng; lựa chọn bị khóa trong phạm vi `teachers` của lớp và payload giữ lại `calendarEventId`/`meetLink` để không mất liên kết recurring meeting đã sync.
