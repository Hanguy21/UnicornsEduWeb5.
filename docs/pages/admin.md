...
- **Giáo án admin (FE `/admin/lesson-plans`, alias `/admin/lessons`):**
  ...
  - Cùng module này hiện được tái dùng thêm dưới staff shell tại `/staff/lesson-plans`, `/staff/lesson-plans/tasks/[taskId]`, `/staff/lesson-manage-details` cho role `lesson_plan_head`; contract UI/API giữ nguyên, chỉ đổi route-base để không thoát khỏi nhóm `/staff`.
  - Có thêm participant workspace dưới `/staff/lesson-plan-tasks`, `/staff/lesson-plan-tasks/[taskId]`, `/staff/lesson-plan-manage-details` cho role `lesson_plan`: dùng cùng shared layout với head workspace/task detail/phóng to, nhưng backend chỉ trả task + resource của các task staff đang tham gia; UI khóa toàn bộ action quản trị task/resource/payment và chỉ giữ create output/resource vào task của chính họ.
  ...
  - API lesson mới:
    ...
    - `GET /lesson-task-staff-options?search=&limit=` trả option nhân sự gọn cho popup chọn `người chịu trách nhiệm` của task; hiện giới hạn tối đa `3` kết quả mỗi lần, search theo `fullName`, và chỉ trả staff có role `lesson_plan` hoặc `lesson_plan_head`
  ...
  - Popup task trong tab `Tổng quan` chỉ cho phép search và chọn `người chịu trách nhiệm`; danh sách `nhân sự thực hiện` trong task là read-only và tự đồng bộ từ các `LessonOutput` con đang gắn task đó.
