...
- **Giáo án admin (FE `/admin/lesson-plans`, alias `/admin/lessons`):**
  ...
  - Cùng module này hiện được tái dùng thêm dưới staff shell tại `/staff/lesson-plans`, `/staff/lesson-plans/tasks/[taskId]`, `/staff/lesson-manage-details` cho role `lesson_plan_head`; contract API giữ nguyên và phần lớn UI được tái dùng, chỉ đổi route-base để không thoát khỏi nhóm `/staff`, đồng thời route task detail dưới staff shell ẩn phần tổng hợp `nhân sự thực hiện output`.
  ...
  - Popup task trong tab `Tổng quan` cho phép search và chọn riêng `người chịu trách nhiệm` và `nhân sự thực hiện task`; khối `nhân sự thực hiện output` là read-only, tổng hợp từ các `LessonOutput` con nhưng không còn ghi đè assignment của task.
