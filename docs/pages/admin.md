...
- **Calendar tuần:** `/admin/calendar` dùng `Class.schedule` làm source of truth và chỉ hiển thị lịch học của **tuần hiện tại** theo layout week-view kiểu Google Calendar, cố định từ **CN -> T7**; FE có **Calendar / Schedule**: Schedule là danh sách nhóm theo ngày, chỉ hiện ngày có slot. UI tối giản: header + bộ lọc gọn (bỏ khối mô tả dài; tooltip/`title` khi cần), lưới giờ co theo buổi (không kéo dải nửa đêm khi chỉ có lịch ban ngày), ô giờ thấp hơn, sự kiện không chồng lớp trong cùng cột (`slotEventOverlap: false`). Filter lớp dạng **multi-select** (combobox tìm theo tên, tick nhiều lớp; lọc lớp áp dụng ở client sau khi tải tuần), kèm filter **gia sư** (`UpgradedSelect`) gửi lên API; không dùng pagination hay chọn khoảng ngày thủ công. Event card và popup không còn hiển thị gia sư của từng buổi; nếu lớp đã sync recurring event lên Google Calendar thì popup đọc trực tiếp `meetLink` từ schedule occurrence để mở link lớp và có thêm icon copy nhỏ để sao chép nhanh link Meet. Google Calendar chỉ áp dụng cho recurring schedule của lớp, không sync riêng các `session` được tạo/sửa/xóa từ màn lớp hoặc staff. Trên mobile, week-view giữ đủ 7 ngày bằng vùng cuộn ngang có chủ đích, filter xếp dọc mobile-first, và popup sự kiện chuyển sang kiểu bottom-sheet cuộn nội bộ để không tràn viewport.
...
- **Class list (FE `/admin/classes`):** Hiển thị các cột chính: Tên lớp, Loại lớp, Sĩ số; dấu chấm trạng thái ở đầu mỗi dòng (running = warning, ended = muted). Dùng TanStack Query gọi `GET /class` qua `apps/web/lib/apis/class.api.ts`; filter search + type đi qua query params backend. Click dòng → `/admin/classes/:id`. Nút **Thêm lớp học** mở popup **Thêm lớp** (UI tối giản: khối gia sư, học sinh, học phí với **Tổng gói** + **Số buổi** và một dòng mức/buổi khi nhập hợp lệ, lịch học); submit `POST /class` gửi `student_tuition_per_session` làm tròn từ gói; success toast + đóng popup + invalidate `['class','list']`. Danh sách hỗ trợ phân trang theo `page` (URL query), điều hướng Trước/Sau, đồng bộ `page` theo `meta.page` từ backend và hiển thị phạm vi bản ghi hiện tại. Hành động xóa lớp dùng popup confirm nội bộ của app thay vì browser confirm/window popup.
