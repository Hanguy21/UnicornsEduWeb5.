...
- **Calendar tuần:** `/admin/calendar` dùng `Class.schedule` làm source of truth và chỉ hiển thị lịch học của **tuần hiện tại** theo layout week-view kiểu Google Calendar, cố định từ **CN -> T7**; giữ filter lớp và giáo viên, không dùng pagination hay chọn khoảng ngày thủ công.
...
- **Sessions endpoints (admin-only):**
  - `POST /sessions` tạo session kèm danh sách attendance. Attendance có `status = present` hoặc `excused` đều bị tính học phí và ghi transaction ví; chỉ `absent` không bị charge.
  - `PUT /sessions/:id` cập nhật session; attendance được đồng bộ theo payload (upsert bản ghi có trong payload, xóa bản ghi cũ không còn trong payload). Khi đổi một học sinh sang `absent`, backend sẽ bỏ học phí của attendance đó và cân lại số dư/transaction tương ứng.
...
- **Staff endpoints & frontend data fetching:**
  ...
  - `GET /staff/:id/income-summary?month=<01-12>&year=<YYYY>&days=<number>` trả summary thu nhập authoritative từ BE cho card tổng tháng/tổng năm; `Tổng năm` dùng field `yearIncomeTotal` đã cộng session, thưởng, trợ cấp thêm, CSKH, giáo án và trợ cấp trợ lí 3% trong năm đang xem. `Ghi cọc` vẫn tách riêng theo năm + danh sách buổi cọc theo lớp.
  ...
  - `GET /staff` trả response dạng `{ data, meta }` với `meta = { total, page, limit }`; mỗi row trong `data` có thêm `unpaidAmountTotal` là tổng tiền chưa thanh toán authoritative từ session `teacher_payment_status = unpaid`, bonus `status = pending`, commission CSKH `customer_care_payment_status = pending/null`, lesson output `payment_status = pending`, và trợ cấp trợ lí 3% `assistant_payment_status = pending/null`.
