...
- **Lesson-plan detail (FE `/admin/lesson_plan_detail/[staffId]`):**
  ...
  - Layout bám cùng nhịp của `FE /staff/customer-care-detail`: hero block 2 cột với phần giới thiệu bên trái và 3 info cards bên phải (**Nhân sự**, **Vai trò**, **Trạng thái**), sau đó mới tới section dữ liệu.
  - Section dữ liệu có 4 card tổng kết: **Tổng sản phẩm**, **Hoàn thành**, **Chờ xử lý**, **Chi phí còn mở**.
  - Danh sách output hiển thị như **table** thực thụ với các dòng: **Tên bài** · **Công việc** · **Ngày** · **Trạng thái** · **Thanh toán** · **Chi tiết**. Mỗi dòng có nút `Xem/Ẩn`; khi mở sẽ render thêm detail-row ngay dưới để xem contest, thời điểm cập nhật, level và các link điều hướng nhanh sang task/output.
