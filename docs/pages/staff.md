...
- **Runtime access hiện tại:**
  ...
  - `/staff/students`, `/staff/students/[id]`: mở cho `assistant` hoặc `accountant_income`; assistant mirror gần như toàn bộ thao tác admin trên học sinh, nhưng không được duyệt queue nạp thẳng và không được nạp tiền thủ công trực tiếp vào ví. Assistant vẫn được rút/giảm số dư trực tiếp khi cần. `accountant_income` được xem thông tin thu/học phí và chỉnh gói học phí học sinh theo lớp (tổng gói + số buổi), nhưng không tạo QR/chỉnh số dư. List `/staff/students` re-export UI admin nên cũng hiện cột/dòng **Lớp** (các lớp `studentClasses` đang active; xem `docs/pages/admin.md`).
  ...
  - `/staff/system-settings`: mở cho `assistant` như admin-mirror (tab **Khấu trừ**, tab **Lương cứng**); các staff role khác vẫn bị chặn. Bookmark cũ `/staff/deductions` redirect tới `/staff/system-settings?tab=deductions`.
  - `/staff/classes/[id]`: `admin`, linked staff có `teacher`, `assistant`, `accountant_income`, `accountant_expense`, `customer_care` khi lớp có ít nhất một học sinh thuộc phạm vi CSKH của staff hiện tại, hoặc `training` khi staff là **quản lý lớp** được gán (`classes.training_manager_staff_id`)
...
- `/staff/calendar`: linked staff có role `teacher`, `training`, hoặc admin đầy đủ; hiển thị lịch read-only với aggregate event feed và 2 chế độ (cùng UX compact với `/admin/calendar` ở phần Calendar/Schedule + multi-select lớp; staff không có filter gia sư hay quyền CRUD buổi bù):
  ...
  - `training` chỉ thấy lớp `running` mà chính họ được gán **quản lý lớp**, lịch cố định, lịch bù và lịch thi thuộc các lớp đó; backend redacts `studentId`/`studentName` và đổi title lịch thi không chứa tên học sinh.
  - filter lớp hỗ trợ multi-select (chọn nhiều lớp cùng lúc); với `teacher` không hiển thị lớp của teacher khác, với `training` chỉ hiển thị lớp đang chạy mà staff là quản lý lớp.
...
- `/staff`
  ...
  - Header hồ sơ trên `/staff/profile`, `/admin/staffs/:id`, và mirror `/staff/staffs/:id` hiển thị avatar nhân sự từ `avatarUrl`; nếu user chưa có avatar thì fallback về chữ cái đầu như trước. Bấm avatar mở popup xem full-size kèm nút **Tải ảnh**; danh sách mirror `/staff/staffs` cũng hiển thị avatar và bấm avatar đi tới hồ sơ chi tiết.
  - `teacher`: thêm 3 khối `Lớp phụ trách`, `Lớp chưa điền lịch / khảo sát`, `Lịch hôm nay`. Khối `Lớp chưa điền lịch / khảo sát` hiện vẫn dùng semantics lịch sử (lần khảo sát N toàn cục, bảng `survey_round.current_round`) cho card dashboard tổng quan; **cảnh báo khảo sát chính** (multi-survey, theo `Survey`/Bài khảo sát mới) hiển thị qua modal `SurveyReminderGate` mỗi lần truy cập `/staff` — xem mục **Bài khảo sát** ở `docs/pages/admin.md`.
  ...
  - `assistant`: thêm cảnh báo hành động kiểu admin (cùng aggregate `GET /dashboard`; **Sắp hết tiền** chỉ học sinh đang học, **Chưa thu** vẫn gồm nợ ví âm kể cả HS đã nghỉ), summary vận hành (`lớp`, `học sinh`, `giáo viên`), khối **CSKH của tôi** (khi user cũng có role `customer_care`) và khối **CSKH tôi quản lí** (chỉ CSKH có `customer_care_managed_by_staff_id` trỏ về trợ lí đang đăng nhập, không gồm self-managed) kèm **Học phí đã học** và **Tiền nạp ví** trong tháng đang xem; thêm khối **Tổng hợp CSKH** (`salesCsSummary` + `salesCsStaffBreakdown` từ `GET /users/me/staff-dashboard`) gồm KPI: tổng HS đang học, HS mới/nghỉ trong tháng đang xem, tổng số HS đang nợ học phí và tổng nợ ví âm; hai danh sách **Tiền nạp ví theo nhân sự** và **Công nợ theo nhân sự** (deep-link `/staff/customer-care-detail/[staffId]`, dòng **(Tôi)** trỏ `/staff/customer-care-detail` khi dual-role). Phạm vi KPI học sinh = học sinh gán cho CSKH do trợ lí quản lí **cộng thêm** portfolio CSKH cá nhân khi hồ sơ trợ lí cũng có role `customer_care`. **Học phí đã học** = tổng `tuition_fee` buổi `present`/`excused` có `sessions.date` trong tháng; **Tiền nạp ví** = tổng `wallet_transactions_history` loại `topup` có `created_at` trong tháng. **Công nợ** = mọi HS được gán CSKH có `account_balance < 0`, không lọc theo trạng thái HS/lớp. **HS nghỉ tháng này** đếm `student_info.drop_out_date` theo **tháng dương lịch** (so sánh biên ngày `DATE` trong SQL, không dùng timestamp), cùng semantics dashboard CSKH self-service; chỉ tính khi `drop_out_date` không null (backend tự đóng dấu khi chuyển **Nghỉ học**). Hai KPI **HS mới tháng này** / **HS nghỉ tháng này** bấm được: mở popup liệt kê tên học sinh + lớp + ngày mới/nghỉ (`GET /users/me/staff-dashboard/customer-care-student-changes?scope=managed`), phạm vi = CSKH mà trợ lí quản lí + portfolio cá nhân nếu dual-role.
  - `customer_care`: thêm số học sinh mới/nghỉ trong tháng, số học sinh đang chăm sóc, **Học phí đã học** và **Tiền nạp ví** trong tháng đang xem, danh sách học sinh số dư thấp và học sinh nợ tiền. Phạm vi đếm là các học sinh đang được gán cho CSKH hiện tại (`customer_care_service.staff_id`): **mới tháng này** đếm `student_info.created_at` thuộc tháng đang xem; **nghỉ tháng này** đếm `student_info.drop_out_date` thuộc tháng đang xem (ngày này được backend tự đóng dấu khi chuyển học sinh sang **Nghỉ học** và tự xoá khi mở lại), nên số liệu phản ánh trực tiếp thao tác đổi trạng thái học sinh thay vì phụ thuộc nhập tay; **đang chăm sóc** đếm theo `status = active`. **Học phí đã học** / **Tiền nạp ví** cùng semantics tháng như khối trợ lí. **Công nợ** = mọi HS được gán có `account_balance < 0`. Mỗi card có subtitle một dòng giải thích phạm vi. Hai stat **mới tháng này** / **nghỉ tháng này** bấm được để mở popup liệt kê tên học sinh + lớp + ngày (`GET /users/me/staff-dashboard/customer-care-student-changes?scope=own`), phạm vi = chỉ học sinh do chính CSKH này phụ trách.
  - `accountant_income`: thêm khối báo cáo thu rút gọn (tổng nạp, học phí đã học, lợi nhuận tháng) cùng nguồn aggregate với admin dashboard; `pendingPayrollTotal` / breakdown thu gồm lương cứng pending giống các khoản trợ cấp khác. Role này **không** thấy lương/trợ cấp gia sư trên lớp; lương cứng cũng không lộ trên class/session redaction.
  - `accountant_expense`: thêm dashboard tổng hợp chi riêng, chỉ hiển thị dữ liệu outbound/payables; gồm KPI tháng `Tổng phát sinh` / `Đã chi` / `Chờ chi`, breakdown chi tháng theo buổi dạy, CSKH, trợ lí, giáo án, bonus, extra allowance, **lương cứng** và chi phí vận hành, backlog pending toàn thời gian theo top 6 nhân sự (gồm lương cứng pending), cùng card chi phí vận hành pending trỏ `/staff/costs?status=pending`
  - `training`: thêm khối Đào Tạo gồm số lớp có lịch hôm nay, số sự kiện hôm nay, tổng lớp đang chạy **được gán quản lý** và tổng khung lịch cố định; CTA mở `/staff/calendar`
...
- `/staff/users`, `/staff/staffs`, `/staff/staffs/[id]`, `/staff/classes`, `/staff/students`, `/staff/students/[id]`, `/staff/costs`, `/staff/history`
  ...
  - `/staff/users` dùng chung page/component với `/admin/users`: popup **Quản lý user**, shortcut **Chi tiết nhân sự/học sinh** trên list, query `?manage=<userId>`, popup **Thêm user**, xóa từng dòng có xác nhận; assistant không gán role admin
  - `/staff/staffs/[id]` và `/staff/students/[id]` có nút **Quản lý tài khoản** trỏ về `/staff/users?manage=<userId>` khi có linked user
  - internal link, pagination, search và các deep-link nội bộ đều giữ route-base `/staff`
  - `/staff/system-settings` là mirror route cho `assistant` (tab **Khấu trừ** và tab **Lương cứng**); `/staff/deductions` redirect vào tab Khấu trừ; kế toán và các staff role khác vẫn không mở được
...
- `/staff/profile`
  ...
  - hiển thị đầy đủ các section cùng contract dữ liệu với admin detail:
    - Hàng đầu dùng `StaffIdentityOverview`: card đồng bộ style các section khác, QR minimal cùng hàng tiêu đề, phần **Google Meet** dùng Meet link cố định của nhân sự, khối **Thành tích** qua `AchievementListEditor` (self mode); đồng bộ với admin staff detail
  ...
  - popup thêm buổi học chỉ còn nằm ở `/staff/classes/[id]`, tiếp tục dùng flow `staff-ops` với các field ngày học, giờ học (**bắt buộc** giờ bắt đầu và giờ kết thúc; giờ kết thúc phải sau giờ bắt đầu; buổi `paid`/`deposit` khóa sửa giờ), toggle **Dạy thử**, **Nhận xét từng học sinh** (md+: bảng **Học sinh** | **Nhận xét** — gia sư 2 cột; mobile card; **bắt buộc** cho HS `present`/`excused`, `absent` không bắt buộc), **Nội dung bài học**, **Bài tập về nhà**, **Tutorial các buổi học** (bắt buộc); thiếu field bắt buộc → Sonner lỗi, không gọi API; roster khi tạo buổi mới chỉ lấy học sinh trạng thái `active`; header popup **không** hiển thị học phí/trợ cấp/trạng thái thanh toán cho gia sư; nút **Copy nhận xét** format Zalo 5 phần (mục HS liệt kê đủ roster, nhận xét nhiều dòng, `excused` luôn ghi `Vắng có phép`, tutorial ở cuối)
  - từ bảng `Lịch sử buổi học` trên `/staff/profile`, staff có thể bấm toàn bộ dòng/card buổi học để mở form chỉnh sửa buổi học, cập nhật lại ngày giờ, toggle **Dạy thử**, nội dung bài học, BTVN, tutorial các buổi học, nhận xét từng HS và điểm danh; header popup chỉnh sửa ẩn học phí/trạng thái thanh toán; khối **Trợ cấp buổi** (nếu hiện) read-only qua `GET /staff-ops/classes/:id`
...
- `/staff/classes/[id]`
  - tab **Nội dung** dùng chung `ClassContentManager` với admin: nút **Thêm chuyên đề** mặc định tab **Thêm từ khoá** (cây Chủ đề → Chuyên đề) khi khoá còn topic; fallback **Tạo mới cho lớp** nếu khoá trống. Lần giao luyện tập: Ngày/Giờ mở bài tuỳ chọn (để trống = mở ngay lúc thêm, `openAt` do backend); thời lượng bắt buộc 1–720 phút.
  ...
  - với `staff.customer_care`, route mở ở chế độ **chỉ xem** khi lớp có ít nhất một học sinh đang do chính staff đó phụ trách từ màn CSKH; các CTA sửa khung giờ, thêm buổi và sửa session đều bị khóa. Card **Danh sách học sinh** hiện cột **Số dư** chỉ cho học sinh trong portfolio CSKH (`customer_care_service`); học sinh khác trong cùng lớp có ô trống (backend omit `accountBalance`). Dual-role **trợ lí + CSKH** dùng admin-like detail và thấy số dư mọi học sinh. Cột **Người chăm sóc** (họ tên CSKH) hiện cho mọi role xem được trang; CSKH thuần được link tên khi người chăm sóc là chính mình (self `/staff/customer-care-detail`), còn tên CSKH khác chỉ chữ thường.
  - với `staff.training`, route mở ở chế độ **chỉ xem** cho các lớp mình được gán quản lý; có thể bấm từng dòng **buổi học** để mở popup chi tiết chỉ đọc, bấm từng **buổi bù** (nếu có) để xem form chi tiết chỉ đọc, và mở popup **khung giờ học** / **khảo sát** ở chế độ xem; không hiện banner cảnh báo quyền hạn khi chỉ xem lịch bù; **không** hiện cột **Số dư** ví trên danh sách học sinh (backend redact `accountBalance`); vẫn thấy cột **Người chăm sóc** (chữ thường, không link).
  ...
  - `teacher` được phân công lớp có thể tạo buổi bù mới trực tiếp từ `/staff/classes/[id]`, nhưng `gia su phu trach` bị khóa cứng về staff hiện tại. Form **Thêm buổi bù** có **Ngày gốc** (DateInput tuỳ chọn, không bắt buộc chọn từ card **Cảnh báo chưa dạy**). Nếu ngày gốc + gia sư khớp cảnh báo chưa dạy, popup hiện textarea **Lý do giải trình** bắt buộc và submit một lần lưu giải trình + tạo buổi bù. Teacher **không** thấy cột **Số dư** ví trên danh sách học sinh; vẫn thấy cột **Người chăm sóc** (chữ thường).
  - **Nội dung lớp (màn 09c):** `ClassContentManager` dùng chung admin/staff. **Tạo mới cho lớp** + luyện tập: hai banner riêng (chuyên đề riêng lớp vs câu hỏi dùng chung cấp khoá); soạn đề trong panel — chọn từ ngân hàng (`ClassPracticeQuestionComposer` BankPicker: `useCourseChapters`/`useCourseDifficultyLevels`, search debounce 300ms, `isLoading` → shadcn `Skeleton`, empty `"Không còn câu hỏi khả dụng"` chỉ sau khi load xong), soạn mới, **✨ Nhập từ AI** (`AiImportModal` variant `inline` từ #53; cổng review tuần tự bắt buộc #102). Mỗi dòng có nhãn `Từ ngân hàng` / `✨ AI · mới` / `✏ Soạn mới`. Tạo topic class-scoped rồi link câu qua API #55; câu mới vào ngân hàng khoá. Dòng luyện tập có **Thống kê** (`/staff/classes/:id/practice/:cid/stats`, Màn 12 / #64, thang 100; click header Điểm / Trạng thái để sort; **Xuất CSV** chỉ toast success sau khi tạo file thành công) và **Chấm tự luận** (`/staff/classes/:id/grading/:assignmentId`, #63; điểm câu = 100/N snapshot; Save disabled hiện **Chưa nhập điểm** / **Điểm vượt thang**). Cron mỗi phút chốt Attempt hết giờ (`timed_out`) nên thống kê không kẹt Chưa làm và tự luận vào hàng đợi dù HS đóng tab. Trang chấm giữ snapshot hàng đợi (không invalidate giữa các câu); hết cursor hiện màn đã chấm xong. Nút **Ẩn** (không xoá DB): `DELETE /class/:id/content/:itemId` set `hiddenAt`; học sinh không còn thấy ở nội dung/timeline. Item đã ẩn hiện badge **Đã ẩn** + **Khôi phục** (`POST /class/:id/content/:itemId/restore`). Toast Sonner.
  ...
  - card `Danh sách học sinh` tách thành 2 phần: danh sách học sinh **đang học** (có cột **Người chăm sóc** + cột **Số dư** theo quyền ở trên) và block riêng **Học sinh đã nghỉ** (không hiện số dư / người chăm sóc) để tránh lẫn trạng thái
  - `GET /staff-ops/classes/:id` trả `customerCareStaff` trên từng student giống `GET /class/:id`; redact `accountBalance` theo access mode: elevated (admin/assistant/kế toán) giữ đủ; `customer_care` chỉ giữ học sinh được gán; `teacher` / `training_manager` bỏ hết.
  ...
  - cho phép thêm `session` với ngày học, giờ học, toggle **Dạy thử**, checkbox **Không cần điểm danh cho buổi này** (default từ `Class.noAttendance`; tick thì ẩn điểm danh, BE auto `present` + vẫn tính phí), `lessonContent`, `homework`, `tutorial`, nhận xét từng HS và điểm danh (bảng 2 cột md+ / card mobile); rich text (`RichTextEditor` TipTap) cho nội dung bài học/BTVN/tutorial/nhận xét HS — paste/autolink URL giữ caret (không sanitize trên controlled sync); nhận xét HS `present`/`excused` bắt buộc khi tạo/lưu (bỏ qua khi buổi không điểm danh)
  ...
  - khối **Lịch sử & Nội dung** là **một timeline** (`ClassTimelineManager`): buổi học + khảo sát + chuyên đề. Bấm dòng mở dialog chi tiết; dòng **Chuyên đề lý thuyết** có CTA **Tiến độ** mở dialog roster `GET /class/:id/content/:itemId/theory-progress` (đã xem + hoàn thành bài tập ôn nhẹ); dòng **Chuyên đề luyện tập** trên staff hiện thêm CTA **Thống kê** (`/staff/classes/:id/practice/:cid/stats`) và **Chấm bài** (`/staff/classes/:id/grading/:assignmentId`) ngay trên row. Kéo-thả đổi thứ tự **trên client**; **Lưu thứ tự** mới gọi `POST /class/:id/timeline/reorder` `{ orderedIds }` (mọi item đúng 1 lần; trùng/id lạ → 400). Lần lưu đầu khóa `timeline_custom_order`. Lưu lỗi: rollback list server, clear dirty, toast Sonner — kéo lại được. Drag handle `touch-none`. CSKH/Đào tạo chỉ xem. Không còn tab Lịch sử/Nội dung hay `MonthNav` trên dải này (tháng vẫn dùng cho lịch bù). Staff gọi `GET /class/:id/timeline` + `staff-ops` khi mở buổi/khảo sát.
  - bảng lịch sử buổi học dùng `SessionHistoryTable` `variant="classDetail"` (ẩn list, mở từ dòng timeline sau khi list buổi theo tháng về); hiển thị `trạng thái thanh toán` chỉ đọc; preview trợ cấp read-only khi sửa buổi
  - khảo sát: teacher được phân công lớp tạo/sửa/xóa báo cáo của mình qua panel ẩn trên timeline; picker `GET /survey-warnings/open-surveys`; `customer_care` chỉ xem. Vẫn nộp sau hạn Bài khảo sát.
...
- `/staff/customer-care-detail`
  ...
  - ở tab **Học sinh**, tên học sinh mở trực tiếp `/staff/students/[id]` và tên lớp mở `/staff/classes/[id]`; backend chỉ trả dữ liệu cho đúng học sinh/lớp thuộc hồ sơ CSKH hiện tại; chi tiết học sinh cho phép CSKH sửa hồ sơ (trừ %), còn chi tiết lớp vẫn read-only theo policy CSKH
  - heading tab **Học sinh** hiển thị `Học sinh (Số lượng: N)`; tab **Thanh Toán** hiển thị `Thanh Toán (tổng thanh toán: …)` từ `meta.totalAmount` của topup-history
  - tab **Hoa hồng** luôn dùng `MonthInput` (mặc định tháng hiện tại): cột **Chưa thanh toán** = pending all-time; cột **Tổng hoa hồng** = commission trong tháng chọn; header bảng hiển thị tổng số học sinh / tổng pending / tổng hoa hồng tháng; panel mở rộng buổi học lọc theo tháng đang chọn
...
- `/staff/assistant-detail`, `/staff/accountant-detail`, `/staff/communication-detail`, `/staff/technical-detail`, `/staff/training-detail`
  ...
  - riêng `/staff/training-detail` (self + admin mirror `?staffId=`): thêm tab **Lớp học** bên cạnh **Trợ cấp** — bảng lớp được gán quản lý với `MonthInput`, cột tổng tháng + chưa thanh toán all-time, deep-link `/staff/classes/[id]`; mở rộng từng lớp để xem chi tiết buổi trong tháng (cùng API `session-allowances` như admin); bulk đổi trạng thái thanh toán buổi chỉ khi actor là admin/assistant/kế toán (training self xem read-only buổi); dữ liệu lớp từ `GET /training-manager/staff/:staffId/managed-classes?month=`
  ...
  - tab **Trợ cấp** có `MonthNav` (mặc định tháng hiện tại); FE gọi `GET /users/me/staff-extra-allowances` kèm `year` + `month` để lọc danh sách và 3 card tổng hợp theo tháng đang xem; popup **Thêm trợ cấp** pre-fill `month` theo tháng đang chọn
  - riêng `/staff/assistant-detail` (self-service): thêm tab **Hoa hồng** bên cạnh **Trợ cấp** để xem phần chia 3% từ CSKH mà trợ lí quản lý theo cấu trúc CSKH → học sinh → buổi học; mặc định filter **Chưa thanh toán**, có thêm filter **Theo tháng**; trợ lí được chọn từng buổi và đổi `assistant_payment_status` qua `PATCH /assistant-commission/staff/:assistantStaffId/payment-status/bulk`; mỗi hàng nhân sự CSKH hiển thị thêm **Số người nợ** và **Tổng tiền nợ** (công nợ học phí hiện tại của học sinh đang active thuộc CSKH đó, cùng semantics dashboard CSKH) ở bên phải row
...
- Assistant admin-mirror routes **được phép**
  ...
  - route `/staff/users` mirror đầy đủ quản trị user cho `assistant`, gồm create/update/delete/gán role qua cùng UI `apps/web/app/admin/users/page.tsx` (re-export). Assistant trên `/staff/users` ẩn option gán `roleType=admin` và `staff.admin` (`hideAdminRoleOptions`); deep-link hồ sơ liên kết giữ prefix `/staff`.
...
- **Backend routes đang dùng**
  ...
  - `GET/PUT /fixed-salary-settings/role-defaults` và `GET/PUT /fixed-salary-settings/role-operating-rates` — tab **Lương cứng** (một bảng 3 cột, hai nút lưu độc lập theo hai API); admin + assistant xem/sửa; không đụng trợ cấp buổi học
  - `GET /fixed-salary-settings/staff-overrides?staffId=`, `PUT /fixed-salary-settings/staff-overrides/amount`, `PUT /fixed-salary-settings/staff-overrides/operating-rate` — đè theo cặp (nhân sự, role). Từ 2026-09-10 UI mức đè **không còn trên tab Lương cứng** mà nằm ở card **Mức đè lương cứng theo nhân sự** trong trang chi tiết nhân sự (`/staff/staffs/[id]`, mirror admin); hai trục độc lập; 0 = cố ý loại; gỡ đè về mặc định role
  - `GET /fixed-salary-settings/payables`, `POST /fixed-salary-settings/close-month` — chốt lương cứng tháng hiện tại + xem khoản đã đóng băng; admin + assistant; cron ngày 28 cùng hàm
  ...
  - `PATCH /staff/:id/payment-status/pay-all` — assistant/accountant_expense trên route mirror có thể thanh toán toàn bộ tập liệt kê trong preview (mọi khoản pending/unpaid mọi role **gồm lương cứng**, trừ cọc); backend tự tính lại trong transaction, refresh snapshot thuế hiện hành theo role **trừ lương cứng** (giữ % đóng băng), rồi chuyển sang `paid`; riêng nguồn giáo viên snapshot cả `% khấu trừ vận hành` hiện hành theo cặp gia sư-lớp và dùng công thức `gross - operating` rồi mới áp thuế
  - `PATCH /staff/:id/fixed-salary-payables/:payableId` — sửa số gộp/`note` khi khoản lương cứng còn pending; net tính lại từ % đóng băng; không có DELETE; khoản paid 400
  ...
  - `GET /users/me/staff-dashboard/customer-care-student-changes?month=&year=&type=new|dropped&scope=own|managed` — danh sách học sinh mới/nghỉ trong kỳ đang chọn cho popup "bấm để xem" ở khối CSKH của `/staff`; `scope=own` chỉ CSKH hiện tại, `scope=managed` gồm CSKH quản lí + self nếu dual-role; mỗi row có `studentId`, `studentName`, `classNames`, `eventDate` (ISO, `createdAt` khi `type=new` hoặc `dropOutDate` khi `type=dropped`)
  ...
  - `PATCH /staff-ops/classes/:id/schedule` — upsert khung giờ học tương tự admin, ghi vào bảng `class_schedule_entries` (xem chi tiết semantics soft-close ở `admin.md`): body thêm `removedEntryIds?: string[]` để soft-close tường minh (`effectiveTo`), `expectedUpdatedAt?: string` để bật optimistic lock — lệch với `Class.updatedAt` hiện tại trả `409`; mỗi slot có thể kèm `effectiveFrom?: string` (`YYYY-MM-DD`) để backdate ngày thay đổi thực sự có hiệu lực; Google sync sau khi lưu giữ nguyên lịch sử (`effectiveTo`) của entry cũ. Trả về danh sách cảnh báo `warnings` cho các buổi bù tương lai. Với `staff.teacher` tự sửa lịch của chính mình: backend chỉ cho phép động tới các slot có `teacherId` là chính họ (cả upsert lẫn `removedEntryIds`), thao tác lên slot của gia sư khác trả `403`. **`effectiveFrom` chỉ Admin/Trợ lý mới được đặt** — nếu actor là gia sư thuần túy (`isTeacherActor`, không kiêm `admin`/`assistant`), backend tự bỏ qua `effectiveFrom` trong mọi slot của payload trước khi ghi (`updateClassScheduleForStaff` → `class.service.ts`), luôn dùng ngày hiện tại bất kể client gửi gì; FE tương ứng cũng ẩn ô "Ngày hiệu lực" khi hiển thị cho gia sư tự sửa lịch (`EditClassSchedulePopup` nhận `allowEffectiveFromEdit={isAdmin}` ở trang `staff/classes/:id`).
  ...
  - `POST /staff-ops/classes/:classId/sessions` — nhận `noAttendance?`; omitted thì default `Class.noAttendance`; `true` thì auto `Attendance.present` và ẩn form điểm danh
  - `PUT /staff-ops/sessions/:id` — cập nhật session; attendance bị bỏ qua nếu session có `snapshotNoAttendance = true` (cờ buổi, không phải cờ lớp live)
...
- card **Lớp phụ trách** dùng `classMonthlySummaries` từ `staff-income-summary`; các cột **Tổng / Chưa nhận / Đã nhận** đều là gross allowance trước CPVH/thuế, trong đó **Chưa nhận** là toàn bộ trợ cấp `unpaid/pending` hiện tại của từng lớp, không còn giới hạn 14 ngày gần nhất; phân công hiện tại luôn hiện; lớp nghỉ dạy chỉ hiện khi còn trợ cấp tháng/`unpaid` và `isCurrentTeacherAssignment=false` → badge **NGHỈ DẠY**
...
- `staff.assistant` vào được `/staff/dashboard` (redirect), `/staff/users`, `/staff/staffs`, `/staff/classes`, `/staff/students`, `/staff/costs`, `/staff/history`, `/staff/system-settings` (bookmark `/staff/deductions` redirect); direct-topup queue và dashboard tổng admin vẫn bị chặn
