...
- `/staff/classes/[id]`
  ...
  - **Nội dung lớp (màn 09c):** `ClassContentManager` dùng chung admin/staff. **Tạo mới cho lớp** + luyện tập: hai banner riêng (chuyên đề riêng lớp vs câu hỏi dùng chung cấp khoá); soạn đề trong panel — chọn từ ngân hàng, soạn mới, **✨ Nhập từ AI** (`AiImportModal` variant `inline` từ #53). Mỗi dòng có nhãn `Từ ngân hàng` / `✨ AI · mới` / `✏ Soạn mới`. Tạo topic class-scoped rồi link câu qua API #55; câu mới vào ngân hàng khoá. Dòng luyện tập có **Thống kê** (`/staff/classes/:id/practice/:cid/stats`, Màn 12 / #64) và **Chấm tự luận** (`/staff/classes/:id/grading/:assignmentId`, #63). Trang chấm giữ snapshot hàng đợi (không invalidate giữa các câu); hết cursor hiện màn đã chấm xong.
