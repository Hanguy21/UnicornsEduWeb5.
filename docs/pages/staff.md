...
- `/staff/classes/[id]`
  ...
  - **Nội dung lớp (màn 09c):** `ClassContentManager` dùng chung admin/staff. **Tạo mới cho lớp** + luyện tập: hai banner riêng (chuyên đề riêng lớp vs câu hỏi dùng chung cấp khoá); soạn đề trong panel — chọn từ ngân hàng, soạn mới, **✨ Nhập từ AI** (`AiImportModal` variant `inline` từ #53; cổng review tuần tự bắt buộc #102). Mỗi dòng có nhãn `Từ ngân hàng` / `✨ AI · mới` / `✏ Soạn mới`. Tạo topic class-scoped rồi link câu qua API #55; câu mới vào ngân hàng khoá. Dòng luyện tập có **Thống kê** (`/staff/classes/:id/practice/:cid/stats`, Màn 12 / #64, thang 100) và **Chấm tự luận** (`/staff/classes/:id/grading/:assignmentId`, #63; điểm câu = 100/N snapshot). Cron mỗi phút chốt Attempt hết giờ (`timed_out`) nên thống kê không kẹt Chưa làm và tự luận vào hàng đợi dù HS đóng tab. Trang chấm giữ snapshot hàng đợi (không invalidate giữa các câu); hết cursor hiện màn đã chấm xong. Nút **Ẩn** (không xoá DB): `DELETE /class/:id/content/:itemId` set `hiddenAt`; học sinh không còn thấy ở nội dung/timeline. Item đã ẩn hiện badge **Đã ẩn** + **Khôi phục** (`POST /class/:id/content/:itemId/restore`). Toast Sonner.
