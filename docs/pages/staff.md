...
- `/staff`
  ...
  - hiển thị đầy đủ các section cùng contract dữ liệu với admin detail:
    ...
    - `Lịch sử buổi học` của chính mình, kèm CTA thêm buổi học mới cho lớp phụ trách
  ...
  - từ section `Lịch sử buổi học` trên `/staff`, teacher/admin có thể bấm `Thêm buổi học`; nếu staff đang phụ trách nhiều lớp, UI mở một bước chọn lớp trước khi vào `AddSessionPopup`
  - popup thêm buổi học trên `/staff` tái dùng flow `staff-ops` và mở các field ngày học, giờ học, `coefficient`, ghi chú, điểm danh; các field tài chính còn lại như `allowanceAmount`, học phí override và mọi khả năng chỉnh `custom allowance` vẫn bị khóa
...
- Staff self page **được phép**
  ...
  - tự thêm và chỉnh sửa buổi học của lớp mình trực tiếp từ `/staff`; backend vẫn tự áp dụng `customAllowance` hiện có của lớp, còn UI self-service chỉ được gửi `coefficient`
...
- flow thêm buổi học ở `/staff` dùng modal chọn lớp ngắn gọn trước khi mở `AddSessionPopup`; popup session chạy với `teacherMode="readOnly"`, `allowFinancialFields=false` và `allowCoefficientField=true`
...
- `/staff` hiển thị thống kê thu nhập, popup ghi cọc, khối thưởng self-service, công việc khác và lịch sử buổi học; teacher/admin có thể mở flow thêm buổi học từ đây
- `/staff` cho thêm thưởng mới của chính mình, nhưng chỉ ở trạng thái `pending`
- `/staff` cho tạo và chỉnh sửa buổi học qua `staff-ops`, cho phép đổi `coefficient` nhưng không cho chỉnh `allowanceAmount`, học phí override hay `custom allowance`
