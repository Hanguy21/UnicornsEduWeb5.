...
- `/staff/classes/[id]`
  ...
  - khối **Lịch sử & Nội dung** là **một timeline** (`ClassTimelineManager`): buổi học + khảo sát + chuyên đề. Bấm dòng mở dialog chi tiết. Kéo-thả đổi thứ tự **trên client**; **Lưu thứ tự** mới gọi `POST /class/:id/timeline/reorder` (lần lưu đầu khóa `timeline_custom_order`). CSKH/Đào tạo chỉ xem. Không còn tab Lịch sử/Nội dung hay `MonthNav` trên dải này (tháng vẫn dùng cho lịch bù). Staff gọi `GET /class/:id/timeline` + `staff-ops` khi mở buổi/khảo sát.
