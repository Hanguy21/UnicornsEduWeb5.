...
- **Trang chi tiết lớp học sinh (`/student/classes/[id]`):** Một **timeline lớp** (không còn 2 tab Lịch sử / Nội dung): buổi học, báo cáo khảo sát, chuyên đề theo `GET /class/:id/timeline/student` (tự động kéo hết các trang cursor ngay khi mở, `sortOrder` sau khi admin/staff **Lưu thứ tự** DnD). Buổi học → dialog nhận xét/recording; khảo sát → dialog đọc đánh giá (không form gia sư); chuyên đề lý thuyết → `/student/classes/[id]/topics/[topicId]`; luyện tập đã `isOpen` → `/student/classes/[id]/assignments/[assignmentId]`; chưa tới `openAt` thì khoá.
  - **Mục lục timeline (`StudentClassTimelineToc.tsx`):** cột mục lục `sticky` bên trái từ `lg` (grid `220px_minmax(0,1fr)`), mobile dùng nút nổi **Mục lục** mở `ResponsiveDialog`. Rail desktop hiển thị toàn bộ item, không có vùng cuộn riêng; dialog mobile dùng `ResponsiveDialogBody` (`flex-1 min-h-0 overflow-y-auto`, `overscroll-behavior: contain`) nên danh sách dài vẫn cuộn được trong panel. Mỗi mục hiện số thứ tự + icon theo loại (buổi học / khảo sát / lý thuyết / luyện tập) + biểu tượng khoá nếu chưa mở; click cuộn mượt tới row (`scroll-mt-24`). Scroll-spy bằng `IntersectionObserver` (`rootMargin: -96px 0px -60% 0px`) highlight mục đang xem và viền primary trên row tương ứng. Vì mục lục cần biết đủ item nên list tự gọi `fetchNextPage` tới khi hết cursor thay vì chờ sentinel.
...
- **Làm bài luyện tập (màn 14):** `/student/classes/[id]/assignments/[assignmentId]` (lobby, làm lại nhiều lượt) và `/.../attempts/[attemptId]`. Đồng hồ từ `startedAt` của học sinh; hết giờ chốt + chấm MCQ, không huỷ. Lúc `start` snapshot đề + thang 100/N; đề 0 câu → lỗi *Đề chưa có câu hỏi, không thể bắt đầu làm bài.* Autosave `PATCH .../answers` hiện trạng thái **Đang lưu…** / **Đã lưu lúc hh:mm** / **Lưu lỗi — thử lại** (Sonner khi fail). `essayAnswer` tối đa 20.000 ký tự — vượt thì hiện lỗi trên ô trả lời, không gọi API. Nộp: flush save trước, dialog xác nhận kèm số câu chưa trả lời; `beforeunload` khi còn thay đổi chưa lưu. Hết giờ vẫn nộp thẳng (không dialog), finalize server-side. `StudentAttemptTimer`: `sticky top-0` trong vùng scroll của `main` (student layout `h-dvh` + `main overflow-y-auto`) để đồng hồ luôn hiển thị khi cuộn câu hỏi; hiển thị mm:ss mượt (tick 250ms); `aria-live="polite"` chỉ đổi theo phút (và "Hết giờ") để screen reader không đọc mỗi giây.
...
- **API (real):**
  ...
  - `PATCH /users/me/student-classes/:classId/attempts/:attemptId/answers` — autosave câu trả lời. `essayAnswer` tối đa 20.000 ký tự (`@MaxLength`); vượt → 400. FE chặn trước và hiện lỗi.
