...
Số block chuẩn = `to − from` của mọi `class_schedule_entries` đang active (`effective_to IS NULL`), phải cùng một bội số 30 phút dương. Slot lệch nhau hoặc không chia hết 30 phút → không suy được.
